...
class VerifMethod(BaseModel):
    """
    The method used for email verification.
    """ # noqa: E501
    type: StrictStr = Field(description="The method used for the email verification.")
    __properties: ClassVar[List[str]] = ["type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Smtp', 'Headless', 'Api', 'Skipped']):
            raise ValueError("must be one of enum values ('Smtp', 'Headless', 'Api', 'Skipped')")
        return value
